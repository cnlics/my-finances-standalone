...
//        String tipDate = new SimpleDateFormat("yyyy-MM-dd").format(daytip.getTipDate());
        String sql = " insert into " + tableName + "(tipDateStr,`week`,money,`desc`,`type`) " +
                "value('" + daytip.getTipDateStr().trim() + "','" + daytip.getWeek().trim() + "'," + daytip.getMoney() + ",'" + daytip.getDesc().trim() + "','"+daytip.getType().trim()+"') ";
    ...
    public void updateTipDate(Integer year, String tableName) {
        ...
        for (Daytip tip : list) {
            String month = StringUtils.substringBefore(tip.getTipDateStr(), "月").trim();
            String day = StringUtils.substringBetween(tip.getTipDateStr(), "月", "日").trim();
